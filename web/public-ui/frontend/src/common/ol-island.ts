...
class OLIslandWaitElement extends OLIslandElementBase {
  private _islandName: string = ''
  ...
  connectedCallback() {
    ...
    this._islandName = this.getAttribute('name') ?? '';
    if (!this._islandName) {
      throw new Error('Island name is not specified');
    }
  }
...
customElements.define('ol-island-wait', OLIslandWaitElement)
