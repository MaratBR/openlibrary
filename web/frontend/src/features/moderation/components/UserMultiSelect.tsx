...
export function UserMultiSelect({
  ...
  value: ModerationUserListEntry[]
  onChange: (users: ModerationUserListEntry[]) => void
  max?: number
}) {
          ...
          <button type="button" className="Select w-full justify-between">
            <span className={value.length ? '' : 'text-secondary-foreground'}>
              {value.length
                ...
                : window._('moderationPortal.loginHistoryPage.allUsers')}
            </span>
        ...
        <PopoverContent className="W[var(--radixPopoverTriggerWidth)] min-w-72 p-0">
          <div className="p-2 border-b border-border">
            <input
              ...
              placeholder={window._('moderationPortal.loginHistoryPage.searchUsers')}
              autoFocus
            />
          </div>
            ...
            {pending ? (
              ...
              results.map((user) => (
                <div
                  ...
                  tabIndex={0}
                  className="w-full flex cursor-pointer items-center gap-3 rounded-sm px-2 py-2 hover:bg-foreground/5 focus-visible:ring-2 focus-visible:ring-primary/45"
                  onClick={() => toggle(user)}
                  onKeyDown={(event) => {
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block truncate font-mono text-xs text-secondary-foreground">
                      {user.id}
                    </span>
                  </span>
                </div>
              ...
              <div className="p-5 text-center text-secondary-foreground">
                {window._('moderationPortal.loginHistoryPage.noUsers')}
              </div>
            )}
          </div>
          <div className="px-3 py-2 border-t border-border text-xs text-secondary-foreground">
            {window._('moderationPortal.loginHistoryPage.selectionLimit', { count: String(max) })}
          </div>
        </PopoverContent>
      </Popover>
      {value.length > 0 && (
