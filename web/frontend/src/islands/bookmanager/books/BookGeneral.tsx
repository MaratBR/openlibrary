...
export function BookGeneral({ book }: { book: ManagerBookDetailsDto }) {
  return (
    <>
      <DashboardContent.Card class="mt-4">
        <div class="flex gap-2">
          <div class="bg-gradient-to-b from-lime-500 to-lime-300 p-6 rounded-2xl text-3xl font-semibold text-white max-w-64">
          ...
          <div class="bg-gradient-to-b p-6 rounded-2xl text-3xl font-semibold max-w-64 bg-highlight">
            <div>{window._('book.chapters', { count: formatNumber(book.chapters.length) })}</div>
          </div>
        </div>

        <div class="flex gap-1 my-4">
          <NavLink className="btn btn--outline btn--primary" to={`/books/${book.id}/edit`}>
