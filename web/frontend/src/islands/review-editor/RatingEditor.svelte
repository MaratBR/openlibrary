<script lang="ts">
    import { calcPerc } from "./ReviewEditor";

    export const { value, onChange, scale = 1, disableHalfPoints = true }: {
        value: number;
        onChange: (value: number) => void;
        scale?: number;
        disableHalfPoints?: boolean;
    } = $props();
...
<div bind:this={rootElement} class="relative cursor-pointer" on:click={handleClick} style={`width:${540*scale}px;height:${100*scale}px`}>
    ...
    <div class="absolute left-0 top-0 ol-star-background ol-star-background--filled h-full" style={`width:${calcPerc(value)}%;background-size:auto ${scale * 100}px`}></div>
</div>