...
create type ol_analytics.bucket_period_type as enum ('all', 'year', 'month', 'week', 'day');
...
create table ol_analytics.bucket (
    ...
    primary key (book_id, metric, bucket_type, bucket_start)
);
...
create table ol_analytics.book_popularity_bucket (
    ...
    updated_at timestamptz not null default now(),
    primary key (book_id, bucket_type, bucket_start)
);
