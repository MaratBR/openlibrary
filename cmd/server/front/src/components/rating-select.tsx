...
export default function RatingSelect({ value, onChange, disabled = false }: RatingSelectProps) {
      ...
      <SelectContent>
        {AGE_RATINGS_LIST.map((rating) => (
          <SelectItem key={rating} value={rating}>
            ...
            <p className="max-w-[600px] text-muted-foreground">{ratings[rating].summary}</p>
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
